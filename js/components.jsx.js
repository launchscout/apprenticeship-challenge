            ...
            return (
                <li className={classes}>
                    <div className="view">
                        ...
                        <label onDoubleClick={this.handleEditStart}><span>{this.props.label}</span></label>
    ...
    var TodoHeader = React.createClass({
        handleValueChange: function(evt) {
            ...
            if (evt.which === 13 && text) { // hit enter, create new item if field isn't empty
                TodoActions.addItem(text + "      $" + prompt("Enter a price: ") + "        SKU: " + prompt("Enter product SKU: "));
//              TodoActions.addItem(text + "      $" + Math.round(Math.random() * 10) + "        SKU: " + Math.round(Math.random() * 100));
                evt.target.value = '';
        ...
        render: function() {
            return (
                <header id="header">
                    <input id="new-todo" placeholder="Shopping List" autoFocus onKeyUp={this.handleValueChange}/>
                </header>
            );
        }
    });
